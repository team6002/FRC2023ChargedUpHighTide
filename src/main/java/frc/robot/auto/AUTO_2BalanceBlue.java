...
public class AUTO_2BalanceBlue extends SequentialCommandGroup {
  public AUTO_2BalanceBlue(AUTO_Trajectories p_trajectories, SUB_Drivetrain p_drivetrain, SUB_Elbow p_elbow, SUB_Elevator p_elevator,
   SUB_FiniteStateMachine p_finiteStateMachine, GlobalVariables p_variables, SUB_Intake p_intake, CommandXboxController p_controller) {
    addCommands(
      // new ParallelRaceGroup(
        ...
        new CMD_IntakeStop(p_intake).withTimeout(3),
        new CMD_setDropLevel(p_variables, GlobalConstants.kElevator3rdLevel).withTimeout(3),
        new CMD_SetInitalOdometry(p_drivetrain, p_trajectories.LinkBalanceTrajectoryBlue).withTimeout(3),
        new ParallelDeadlineGroup(
          new SequentialCommandGroup(
          ...
          p_trajectories.driveTrajectory(p_trajectories.LinkBalanceTrajectoryBlue)
          ),
