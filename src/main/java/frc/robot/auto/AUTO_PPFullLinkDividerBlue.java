...
public class AUTO_PPFullLinkDividerBlue extends SequentialCommandGroup {
  /** Creates a new AUTO_PPFullLinkDivider. */
  AUTO_Trajectories m_trajectories;
  public AUTO_PPFullLinkDividerBlue(AUTO_Trajectories p_trajectories, SUB_Drivetrain p_drivetrain, SUB_Elbow p_elbow, SUB_Elevator p_elevator,
  SUB_FiniteStateMachine p_finiteStateMachine, GlobalVariables p_variables, SUB_Intake p_intake, SUB_IntakeCamera p_intakeCam, CommandXboxController p_controller){
    ...
    addCommands(
      new CMD_SetInitalOdometry(p_drivetrain, p_trajectories.LinkRunTrajectoryBlue1).withTimeout(3),
      ...
      new ParallelCommandGroup(
        ...
        new WaitCommand(.2),
        m_trajectories.followTrajectoryCommand(m_trajectories.CubeRunBlueDivider).withTimeout(2)
          // )
        ),
      ...
      new ParallelCommandGroup(
        ...
        new ParallelDeadlineGroup(
          // new CMD_CheckIntakeCameraTarget(p_intakeCam),
          m_trajectories.followTrajectoryCommand(m_trajectories.ConeRunBlueDivider).withTimeout(2)
        )
      ),
        new CMD_AUTOGroundCubeIntake(p_intake, p_elbow, p_elevator, p_finiteStateMachine).withTimeout(3),
      ...
      new ParallelCommandGroup(
        m_trajectories.followTrajectoryCommand(m_trajectories.ConePlaceBlueDivider),
        new SequentialCommandGroup(
          new CMD_GroundHold(p_intake, p_elbow, p_elevator, p_finiteStateMachine, p_variables).withTimeout(3),      
          new CMD_Place1stLevel(p_intake, p_elbow, p_elevator, p_finiteStateMachine, p_variables).withTimeout(3) 
        )
